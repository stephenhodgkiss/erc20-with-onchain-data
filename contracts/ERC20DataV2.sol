 ...
 *
 * The inclusion of a "onchainData" argument on each external method allows it to write
 * extra on-chain-data to the chain alongside each token transaction, in order to correlate
 * it with other on- and off-chain events you might need.
 */
